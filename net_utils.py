...
class NetworkBase(nn.Module):
    ...
    def fit(self, train_fn, *args, **kwargs):
        ...
        if self.weightReg is not None:
            init_string += "// Weight reg: {}, coef: {:.1e} ".format(
                self.weightReg, self.regLambda
            )
        else:
        ...
        if self.paramSimilarity is not None:
            init_string += "// Param sim: {}, coef: {:.1e} ".format(
                self.paramSimilarity, self.paramSimLambda
            )

        folder = os.path.normpath(kwargs.pop("folder", ""))
        filename = kwargs.pop("filename", None)
        if filename is not None and not os.path.commonprefix(
            [os.getcwd()[::-1], folder[::-1]]
        ):
    ...
    def _train_epoch(
        ...
        batchSize=1,
        earlyStop=True,
        earlyStopValid=False,
        validStopThres=None,
        trainOutputMask=None,
        ...
        for b in range(
            ...
            if (
                ...
                if (
                    earlyStopValid
                    and (
                        len(self.hist["iters_monitor"])
                        - self.hist["monitor_thresh"][-1]
                    )
                    > 2 * STEPS_BACK
                ):
                    if (
                        ...
                        print(
                            "  Early Stop: avg_valid_loss saturated, current (-1): {:.2e}, prev (-{}): {:.2e}, acc: {:.2f}".format(
                                self.hist["avg_valid_loss"][-1],
                                STEPS_BACK,
                                self.hist["avg_valid_loss"][-STEPS_BACK],
    ...
    def evaluate(
        self, batch
    ):  # TODO: figure out nice way to infer shape of y and don't pass in
        pass
    ...
    def accuracy(self, batch, out=None, outputMask=None):
        ...
        if outputMask is None:
            ...
            masked_y = batch[1][outputMask[:, :, 0]].squeeze(
                -1
            )  # [B*T_mask, 1] -> [B*T_mask]
    ...
    def average_loss(self, batch, out=None, outputMask=None):
        ...
        if self.paramSimilarity is not None:
            ...
            dists = torch.sum(
                torch.triu(
                    torch.cdist(param_vecs, param_vecs, p=2).squeeze(0), diagonal=1
                )
            )  # (1, N, N) before squeeze
    ...
    def _monitor_init(
        ...
        if self.hist is None:  # Initialize self.hist if needed
            self.hist = {}
        if (
            "epoch" not in self.hist
        ):  # This allows self.hist to be initialized in children classes first, and then all default keys are added here
            self.hist["epoch"] = 0
            self.hist["iter"] = -1  # gets incremented when _monitor() is called
            self.hist["iters_monitor"] = []
            self.hist["monitor_thresh"] = [
                0,
            ]  # used to calculate rolling average loss/accuracy
    ...
    def _monitor(
        ...
        if (
            ...
            if loss is None:
                loss = self.average_loss(
                    trainBatch, out, outputMask=trainOutputMaskBatch
                )
            if acc is None:
            ...
            if hasattr(self, "writer"):
                self.writer.add_scalar(
                    "train/loss", loss.item(), global_step=self.hist["iter"]
                )
                self.writer.add_scalar(
                    "train/acc", acc.item(), global_step=self.hist["iter"]
                )
                self.writer.add_scalar(
                    "info/grad_norm", gradNorm, global_step=self.hist["iter"]
                )
            ...
            if validBatch is not None:
                ...
                valid_loss = self.average_loss(
                    validBatch, out=valid_out, outputMask=validOutputMask
                )
                ...
                self.hist["avg_valid_loss"].append(
                    np.mean(self.hist["valid_loss"][-avg_window:])
                )
                self.hist["avg_valid_acc"].append(
                    np.mean(self.hist["valid_acc"][-avg_window:])
                )

                if hasattr(self, "writer"):
                    self.writer.add_scalar(
                        "validation/acc", acc.item(), global_step=self.hist["iter"]
                    )
                    self.writer.add_scalar(
                        "validation/acc", acc.item(), global_step=self.hist["iter"]
                    )
                displayStr += " valid_loss:{:.4f} valid_acc:{:.3f}".format(
    ...
    def grad_norm(self):
        return (
            torch.cat([p.reshape(-1, 1) for p in self.parameters() if p.requires_grad])
            .norm()
            .item()
        )
...
def train_dataset(
    ...
    if net.newThresh:
        ...
        trainOutputMaskBatch = (
            trainOutputMask[0:batchSize, :, :] if trainOutputMask is not None else None
        )
        net._monitor_init(
            trainData[0:batchSize, :, :],
            validBatch=validBatch,
            trainOutputMaskBatch=trainOutputMaskBatch,
            validOutputMask=validOutputMask,
        )
    while net.hist["epoch"] < epochs:
        net.hist["epoch"] += 1
        converged = net._train_epoch(
            trainData,
            validBatch=validBatch,
            batchSize=batchSize,
            validStopThres=validStopThres,
            earlyStopValid=earlyStopValid,
...
def train_infinite(net, gen_data, iters=float("inf"), batchSize=None, earlyStop=True):
    trainBatch = (
        gen_data()[:, 0, :] if batchSize is None else gen_data()[:, :batchSize, :]
    )
    ...
    while net.hist["iter"] < iters:
        trainCache = (
            gen_data()
        )  # generate a large cache of data, then minibatch over it
        converged = net._train_epoch(
            trainCache, batchSize=batchSize, earlyStop=earlyStop
        )
...
def train_curriculum(
    ...
    R0=1,
    Rf=float("inf"),
    increment=lambda R: R + 1,
):
    R = R0
    trainBatch = (
        gen_data(R)[:, 0, :] if batchSize is None else gen_data(R)[:, :batchSize, :]
    )
    net._monitor_init(trainBatch)
...
def binary_classifier_accuracy(out, y):
    """Accuracy for binary-classifier-like task"""
    return (
        (out.round() == y.round()).float().mean()
    )  # round y in case using soft labels
...
class Autosave:
    ...
    def __call__(self, force=False):
        if self.filename is not None and (
            force or time.time() - self.lastSave > self.saveInterval
        ):
            self.filename = self.net.save(
                self.filename, overwrite=False if self.lastSave < 0 else True
            )
