...
class HebbNet(StatefulBase):
    def __init__(self, init, verbose=True, **hebbArgs):
        ...
        if self.trainableState0:
            ...
            init_string += "\n    A0: zeros"
            self.register_buffer(
                "A0", torch.zeros_like(torch.tensor(W2[0], dtype=torch.float))
            )
        ...
        if self.forceAnti:
            ...
            if self.freezeInputs:
                ...
                if self.hebbType == "inputOutput":
                    ...
                    self.hebbType == "input"
                ):  # Different eta size since update terms are a slightly different magnitude in this case
                    self.register_buffer("_eta", torch.tensor(1.0, dtype=torch.float))
                    # self.register_buffer('_eta', torch.tensor(1.0/np.sqrt(self.n_hidden), dtype=torch.float))
                elif (
                    self.hebbType == "output"
                ):  # Different eta size since update terms are a slightly different magnitude in this case
                    self.register_buffer(
                        "_eta",
                        torch.tensor(1.0 / np.sqrt(self.n_inputs), dtype=torch.float),
                    )
                self.eta = self._eta
            else:
    ...
    def update_hebb(self, pre, post, isFam=False, stateOnly=False):
        ...
        if self.plastic:
            ...
            if self.modulation_bounds:
                if self.updateType != "hebb" or (
                    self.updateType == "hebb" and self.AAct is not None
                ):
                    raise NotImplementedError(
                        "Modulation bounds only implemented for Hebb updates with no activation."
                    )
            ...
            if self.groundTruthPlast:  # and isFam: # only decays hebbian weights
                ...
                self.updateType == "hebb_norm"
            ):  # normal hebbian update with normalization
    ...
    def build_M_bounds(self):
        ...
        if self.stpType == "add":
            ...
            MAX_MULT = (
                self.mod_bound_val
            )  # Controls how much a weight can be enhanced, 1.0 means the weight's mag can be doubled
            MIN_MULT = (
                -1 * self.mod_bound_val
            )  # Controls how much a weight can be depressed, -1.0 means it can be fully depressed
            M_bounds = torch.cat(
                (
...
class VanillaRNN(StatefulBase):
    def __init__(
        ...
        if (
            self.freezeInputs
        ):  # Does not train input layer or hidden bias (just sets thte latter to zero)
            init_string += "\n  Winp: Frozen //  "
            self.register_buffer("w1", torch.tensor(W[0], dtype=torch.float))
...
class nnLSTM(VanillaRNN):
    ...
    def __init__(
        self, init, verbose=True, f=None, fOut=torch.sigmoid, **rnnArgs
    ):  # f is ignored. Included to have same signature as VanillaRNN
        super(nnLSTM, self).__init__(
            init, resetState=False, netType="LSTM", verbose=verbose, **rnnArgs
        )
...
class FeedForward(StatefulBase):
    def __init__(
        self, init, verbose=True, resetState=True, netType="FeedForward", **rnnArgs
    ):
    ...
    def forward(self, x, debug=False, stateOnly=False):
        ...
        # (1, Nh) + [(B, Nx) x (Nx, Nh)] + [(B, Nh) x (Nh, Nh)] = [B, Nh]
        if self.sparsification > 0:
            a1 = self.b1.unsqueeze(0) + torch.mm(
                x, torch.transpose(self.w1Mask * self.w1, 0, 1)
            )
        else:
            a1 = self.b1.unsqueeze(0) + torch.mm(x, torch.transpose(self.w1, 0, 1))
